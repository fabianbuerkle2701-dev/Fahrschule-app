...
// Bild-zu-Base64-Muster), nur mit kleinerem Ausgabe-Schema: liest ein Foto eines Belegs/
// einer Quittung (z.B. Prüfungsgebühr, Lehrmaterial) aus und schlägt Bezeichnung + Betrag
// für einen neuen freien Kostenposten (students.data.costItems) vor.
...
exports.handler = async function (event) {
  ...
  const instruction = `Du erhältst ein Foto oder einen Screenshot eines Kassenbons, einer Quittung oder einer Rechnung, die eine Fahrschule als freien Kostenposten bei einem Fahrschüler erfassen möchte (z.B. Prüfungsgebühr, Lehrmaterial, TÜV-Gebühr). Lies die sichtbaren Angaben sorgfältig aus und gib AUSSCHLIESSLICH ein JSON-Objekt zurück, ohne Erklärung, ohne Markdown, ohne Backticks.

Das JSON hat genau diese Felder (fehlender Wert als leerer String "" bzw. null bei der Zahl, niemals raten):
{
  "label": "",   // kurze Bezeichnung des Postens, z.B. "Prüfungsgebühr Theorie", "Lehrmaterial", "TÜV-Gebühr" - aus dem Beleg abgeleitet, nicht wortwörtlich der ganze Bon-Text
  "amount": null, // Gesamtbetrag als Zahl mit Punkt als Dezimaltrennzeichen, z.B. 22.80 - der ENDBETRAG des Belegs, nicht ein Zwischenbetrag
  "date": ""     // Datum auf dem Beleg, Format TT.MM.JJJJ, falls erkennbar sonst ""
}

Wichtig: Wenn mehrere Positionen auf dem Beleg stehen, nimm für "label" eine kurze, sinnvolle Zusammenfassung (z.B. "Lehrmaterial" statt jeden einzelnen Artikel aufzuzählen) und für "amount" den GESAMTBETRAG. Wenn der Betrag nicht eindeutig lesbar ist, lieber null als raten. Gib nur das JSON zurück.`;
  ...
  try {
    ...
    return { statusCode: 200, headers, body: JSON.stringify({ receipt: parsed }) };
  } catch (e) {
